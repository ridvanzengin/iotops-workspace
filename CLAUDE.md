...
- **Milestone status**: IoTOps v1 (Milestones 0-4) is shipped. Milestone 5 (Automation Engine, v1.1) is substantially done — real rule/Redis/Celery logic in both custom-telegraf plugins, IoTOps's Automater backend + frontend, automated tests in both repos, and a persisted events feature (Mongo-backed, SSE-delivered) are all shipped. See `../IoTOps/docs/development-plan.md` for the authoritative status and `ROADMAP.md` below for what's actively in flight.
